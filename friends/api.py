...
def send_friend_request_view(request):
    ...
    try:
        ...
        FriendRequest.objects.create(sender=request.user, receiver=receiver_account)
        return Response("Friend request sent")
    except FriendRequest.MultipleObjectsReturned:
        return Response(
            "You have already sent friend request", status=HTTP_409_CONFLICT
        )
    else:
        return Response(
            "You have already sent friend request", status=HTTP_409_CONFLICT
...
def accept_friend_request_view(request, pk):
    ...
    friend_request: FriendRequest = get_object_or_404(FriendRequest, pk=pk)
    if friend_request.receiver != request.user:
        return Response(
            "You don't have permission for this step", status=HTTP_403_FORBIDDEN
        )
    send_notification(friend_request.accept(), friend_request.sender.pk)
...
def decline_friend_request_view(request, pk):
    ...
    send_notification(friend_request.decline(), friend_request.sender.pk)
